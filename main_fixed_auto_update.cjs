...
function logServer(message, isError = false) {
  writeLog(isError ? "server:error" : "server", message);
}
...
autoUpdater.on("update-not-available", (info) => {
  const version = String(info?.version || "");
  logUpdater(`update-not-available latest=${version} current=${app.getVersion()}`);
  updateState = {
    ...updateState,
    checking: false,
    available: false,
...
ipcMain.handle("app:quit-and-install-update", async () => {
  try {
    ...
    setTimeout(() => {
      try {
        logUpdater("calling autoUpdater.quitAndInstall(false, true)");
